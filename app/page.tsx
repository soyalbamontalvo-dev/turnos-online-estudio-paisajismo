    ...
    <main className="page-shell">
      ...
      <section className="booking-card calendar-card" aria-labelledby="calendar-title">
        ...
        {!ready || !weekStart ? <div className="calendar-loading">Preparando disponibilidad…</div> : <>
          <div className="week-toolbar" aria-label="Navegación semanal"><button type="button" className="week-nav-button" aria-label="Semana anterior" onClick={() => changeWeek(-1)} disabled={weekOffset === 0}>←</button><div><span>Semana</span><strong>{weekLabel}</strong></div><button type="button" className="week-nav-button" aria-label="Semana siguiente" onClick={() => changeWeek(1)} disabled={weekOffset === 4}>→</button></div>
          <div className="day-strip">{weekDays.map((date) => { const key = toDateKey(date); const active = key === selectedDate; return <button type="button" className={`day-button ${active ? "is-active" : ""}`} key={key} aria-pressed={active} disabled={key < todayKey} onClick={() => selectDay(date)}><span>{date.toLocaleDateString("es-ES", { weekday: "short" }).replace(".", "")}</span><strong>{date.getDate()}</strong><small>{date.toLocaleDateString("es-ES", { month: "short" }).replace(".", "")}</small></button>; })}</div>
          <div className="availability-heading"><div><span>Horarios de {selectedProfessional.name} · Tegucigalpa (UTC−6)</span><strong>{selectedDateLabel}</strong></div><span className="availability-note">Horarios pasados, citas ocupadas y bloqueos internos no se pueden seleccionar</span></div>
      ...
      {confirmedAppointment && <section className="confirmation-card" id="confirmation" aria-live="polite"><span className="confirmation-icon">✓</span><p className="overline">Reserva confirmada</p><h2>Tu visita ya está en la agenda.</h2><p>Guarda este código para consultar, cancelar o reprogramar tu cita.</p><div className="reservation-code">{confirmedAppointment.code}</div><div className="confirmation-details"><span>{services.find((service) => service.id === confirmedAppointment.serviceId)?.title}</span><strong>{fromDateKey(confirmedAppointment.date).toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long" })} · {confirmedAppointment.time}</strong><span>{professionals.find((professional) => professional.id === confirmedAppointment.professionalId)?.name}</span></div><div className="confirmation-actions"><a className="secondary-button" href={`/gestionar?codigo=${encodeURIComponent(confirmedAppointment.code)}`}>Gestionar esta reserva</a><button type="button" className="secondary-button" onClick={startNewBooking}>Hacer otra reserva</button></div></section>}
