...
export default function ManageBookingPage() {
  ...
  useEffect(() => {
    const today = new Date(); today.setHours(0, 0, 0, 0); setTodayKey(toDateKey(today));
    const stored = window.localStorage.getItem(APPOINTMENTS_KEY);
    if (stored) { try { setAppointments(JSON.parse(stored) as DemoAppointment[]); } catch {} }
    else { const initial = createDemoAppointments(startOfWeek(today)); window.localStorage.setItem(APPOINTMENTS_KEY, JSON.stringify(initial)); setAppointments(initial); }
    try { setBlocks(JSON.parse(window.localStorage.getItem(BLOCKS_KEY) || "[]") as AvailabilityBlock[]); } catch { window.localStorage.removeItem(BLOCKS_KEY); }
  ...
  const availableDays = useMemo(() => {
    if (!todayKey) return [];
    return Array.from({ length: 18 }, (_, index) => addDays(fromDateKey(todayKey), index)).filter((date) => date.getDay() !== 0).slice(0, 12);
  }, [todayKey]);
  ...
  function findBooking(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(""); setMessage(""); setReprogramming(false); setNewDate(""); setNewTime("");
    const normalized = code.trim().toUpperCase(); if (!normalized) return setError("Introduce el código de reserva.");
    let latest = appointments; try { latest = JSON.parse(window.localStorage.getItem(APPOINTMENTS_KEY) || "[]") as DemoAppointment[]; } catch {}
    setAppointments(latest); const found = latest.find((appointment) => appointment.code.toUpperCase() === normalized);
  ...
  function cancelBooking() {
    ...
    const next = appointments.map((a) => a.id === managed.id ? { ...a, status: "cancelado" as const } : a);
    persist(next); setReprogramming(false); setMessage("Reserva cancelada. El horario anterior ya vuelve a estar disponible.");
  }
  function startReprogramming() {
    if (!managed || managed.status === "cancelado") return;
    setError(""); setMessage(""); setNewDate(managed.date >= todayKey ? managed.date : todayKey); setNewTime(managed.time); setReprogramming(true);
  }
  function saveReprogramming() {
    ...
    persist(next); setError(""); setMessage("Reserva reprogramada. El hueco anterior se ha liberado automáticamente."); setReprogramming(false);
  }
